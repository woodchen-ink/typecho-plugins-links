...
/** 插入数据 */
$insertId = $this->db->query($this->db->insert($this->prefix.'links')->rows($link));



// 		$link['lid'] = $this->db->query($this->db->insert($this->prefix.'links')->rows($link));
        /** 插入数据 */
        // $this->db->query($this->db->insert($this->prefix.'links')->rows($link));

        // /** 获取刚插入的数据的lid */
        // $link['lid'] = $this->db->lastInsertId();
...
/** 设置高亮 */
$this->widget('Widget_Notice')->highlight('link-'.$insertId);
